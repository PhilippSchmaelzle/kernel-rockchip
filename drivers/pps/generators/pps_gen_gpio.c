...
MODULE_AUTHOR("Damon Kwok <damon-kwok@outlook.com>");
MODULE_DESCRIPTION(DRVDESC);
MODULE_LICENSE("GPL");

#define GPIO_PULSE_WIDTH_DEF_NS (10 * NSEC_PER_USEC)    /* 30us */
#define GPIO_PULSE_WIDTH_MAX_NS (20 * NSEC_PER_USEC)   /* 100us */
...
static enum hrtimer_restart hrtimer_callback(struct hrtimer *timer)
{
	...
	/*
	const long time_gpio_assert_ns = 1000;//NSEC_PER_SEC - devdata->gpio_instr_time; 
	const long time_gpio_deassert_ns = time_gpio_assert_ns + gpio_pulse_width_ns;
		*/
	...
	/* Assert PPS GPIO. */
	pr_info("pps_gen_gpio: Set Pin HIGH");
